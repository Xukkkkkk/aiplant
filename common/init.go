...
func printHelp() {
	fmt.Println("Nebula Matrix AI Gateway " + Version + " - High Performance Unified AI Model Distribution Gateway")
	fmt.Println("Usage: server [--port <port>] [--log-dir <log directory>] [--version] [--help]")
}
...
func InitEnv() {
	...
	RegisterIPDailyLimit = GetEnvOrDefault("REGISTER_IP_DAILY_LIMIT", 5)
	RegisterIPShortLimit = GetEnvOrDefault("REGISTER_IP_SHORT_LIMIT", 2)
	EmailVerificationEnabled = GetEnvOrDefaultBool("EMAIL_VERIFICATION_ENABLED", false)
	initConstantEnv()
}
